...
\runningheadrule
\firstpageheader{CS559: Machine Learning}{Name:        }{\textcolor{red}{Due: April 17, 2023}}
...
\end{figure}
\question{\bf Bayesian Networks}\label{q2} (10 points)
...
\end{figure}
\question{\bf K-means}\label{q3} (30 points)  Given the matrix $X$ whose rows represent different data points, you are asked to perform a k-means clustering on this dataset using the Euclidean distance as the distance function. Here $k$ is chosen as $3$. The Euclidean distance d between a vector $x$ and a vector $y$ both in $\mathcal{R}^d$ is defined as $d(\xx, \yy) = \sqrt{\sum_{i=1}^d (x_i - y_i)^2}$. All data in X were plotted in Figure~\ref{fig:kmeans}. The centers of $3$ clusters were initialized as $\mu_1= (6.2, 3.2) (\text{red}), \mu_2 = (6.6, 3.7) (\text{green)}, \mu_3 = (6.5, 3.0) (\text{blue})$.

\begin{parts}
\part (24 pts) Implementation of K-means (submit your code).
\part (1.5 pts)  What’s the center of the first cluster (red) after one iteration? (Answer in the format of $[x_1,x_2]$, round your results to three decimal places)
\vspace{5em}
\part (1.5 pts) What’s the center of the second cluster (green) after two iteration?
\vspace{5em}
\part (1.5 pts) What’s the center of the third cluster (blue) when the clustering converges?
\vspace{5em}
\part (1.5 pts)  How many iterations are required for the clusters to converge?
\end{parts}

\newpage
\question{\bf Expectation Maximization (EM)}\label{q4} (25 points)
...
\part (5 pts) \textbf{Maximization of Means}: 
...
\part (5 pts) \textbf{Maximization of Covariances}:
...
\part (5 pts) \textbf{Maximization of Mixture Weights} :
